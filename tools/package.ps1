...
dotnet build installer/Product.wixproj -c Release -t:Rebuild -p:Version=$Version -p:AcceptEula=$AcceptEula -p:PayloadRoot=$PayloadRoot -o "$OutputDirectory\msi"
...
dotnet build installer/Bundle.wixproj -c Release -t:Rebuild -p:Version=$Version -p:AcceptEula=$AcceptEula -p:MsiPath="$OutputDirectory\Akshara-Windows-v$Version.msi" -o "$OutputDirectory\bundle"
